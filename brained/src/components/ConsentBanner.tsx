    ...
    <div className="fixed bottom-0 inset-x-0 z-50">
      <div className="mx-auto max-w-5xl bg-white shadow-lg border rounded-t p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="text-sm text-gray-700">
          We use analytics to improve your experience. We may record clicks, scrolls, and cursor movement. No sensitive input values are collected.
        </div>
        <div className="flex gap-2">
          <button onClick={deny} className="px-3 py-2 rounded border">Decline</button>
          <button onClick={grant} className="px-3 py-2 rounded bg-blue-600 text-white">Allow analytics</button>
        </div>
      </div>
    </div>
  );
